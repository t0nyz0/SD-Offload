...
final class PhotoIndexTests: XCTestCase {
    ...
    func testLegacyGPSKeysDecodeWithoutError() async {
        // A pre-cleanup index JSON with the retired `location`/`gpsChecked` keys
        // must still load — Codable ignores unknown fields, so the record
        // decodes and search still works.
        let legacy = """
        [{"path":"/nas/a.jpg","size":1000,"mtime":"2026-01-01T00:00:00Z",\
        "labels":[],"animals":["Dog"],"analyzedAt":"2026-01-01T00:00:00Z",\
        "location":{"lat":30.5,"lon":-87.9},"gpsChecked":true}]
        """
